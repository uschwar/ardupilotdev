...
// added restart_nav() option at the end of navigation
void save_wp_index()
{
	eeprom_busy_wait();
...
void reached_waypoint()
{
	...
	if (control_mode == AUTO){
		...
		if(wp_index > wp_total){
                        if(CLOSED_LOOP_NAV == 1) {
  			     restart_nav(); // at the end of the nav, closed loop navigation
                        }else{ 
                          set_mode(RTL);    // else return to launch
                        }
       			return;
		}else{
	...
	if (control_mode == HEADALT){  // Max distance reach during the Headalt so RTL
           headalt_set = false;
           set_mode(RTL);            // return to launch
          }           
...
void init_home()
{
	...
	Serial.print(home.lat); 
        Serial.print("  lon:");
        Serial.print(home.lng);
        Serial.print("  alt:"); 
        Serial.println(home.alt);
