...
//****************************************************************
// Updated on 04-12-11 JLN - Use of PID library
...
void stabilize()
{
	...
	// ---------------------------------------------
	servo_out[CH_ROLL]  = PIDcompute((nav_roll - roll_sensor), deltaMiliSeconds, CASE_SERVO_ROLL);
	servo_out[CH_PITCH] = PIDcompute((nav_pitch + abs(roll_sensor * PITCH_COMP) - pitch_sensor), deltaMiliSeconds, CASE_SERVO_PITCH);  
...
void calc_throttle()
{
	...
	// lower the GSBOOST constant value to limit the effect : 50 = default
	int gs_boost = GSBOOST * (1.0 - ((float)ground_speed/100) / (float)airspeed_cruise);
...
void calc_throttle()
{
	...
	// -------------------------------------------
	if(airspeed_error >= 0){
		energy_error = (.5 * airspeed_error * airspeed_error) + (float)altitude_error * 0.098f;
	}else{
		energy_error = (-.5 * airspeed_error * airspeed_error) + (float)altitude_error * 0.098f;
	}
	// positive energy errors make the throttle go higher
	servo_out[CH_THROTTLE] = throttle_cruise + PIDcompute(energy_error, deltaMiliSeconds, CASE_TE_THROTTLE);
	servo_out[CH_THROTTLE] = max(servo_out[CH_THROTTLE], 0);
...
void calc_nav_pitch()
{
	...
	nav_pitch = PIDcompute(altitude_error, deltaMiliSeconds, CASE_NAV_PITCH_ALT);
...
void calc_nav_roll()
{
	...
	// ----------------------------------------
	nav_roll = PIDcompute(bearing_error, deltaMiliSeconds, CASE_NAV_ROLL);	//returns desired bank angle in degrees*100
...
float PIDcompute(long PID_error, long dt, int PID_case)
{
	...
	pid.kI(ki[PID_case]);
	pid.kD(kd[PID_case]);
	pid.imax(integrator[PID_case]);
        //long error = PID_error;
        
        //long control = pid.get_pid(PID_error, dt, 1);
        output = float(pid.get_pid(PID_error, dt, 1));
