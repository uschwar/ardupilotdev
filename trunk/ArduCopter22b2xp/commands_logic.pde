...
static void process_nav_command()
{
	switch(command_nav_queue.id){
		...
		case MAV_CMD_NAV_LAND:	// 21 LAND to Waypoint
			yaw_mode 	= LOITER_YAW;            // JLN update
...
static bool verify_must()
{
	...
	switch(command_nav_queue.id) {
		...
		case MAV_CMD_NAV_LAND:          // JLN update
			if (verify_land()) {			
  		            landing_boost = 180;  
                            nav_throttle--;
                            if(verify_landed()) {
...
static void do_loiter_turns()    // v2.2 b2 and b2xp
{
...
/*
static void do_loiter_turns()      // v2.1.1 r9
{
	wp_control = CIRCLE_MODE;
...
static bool verify_land()      // JLN update for a Kisssss landing
{      int16_t delta2 = 0, lb_offset=0, old_lb=0;
             
        delta2 = baro_alt - takeoff_point.alt;
	// landing detector
	if (delta2 > 300){
		old_alt = baro_alt;
                wp_control = LOITER_MODE;
                ...
                landing_boost++;
                lb_offset = landing_boost;
           	}              
	
	// remember altitude for climb_rate
	old_alt = baro_alt;
	...
	if (delta2 < 30 ){    
		...
		roll_pitch_mode = ROLL_PITCH_STABLE;
                touchdown =  true;
                lb = (int)((175 * icount * icount /100) - (72 * icount / 10));
                lb = constrain(lb, 0, 180);
                landing_boost = (int)lb + lb_offset;
                landing_boost = max(landing_boost, old_lb);
                icount = icount + 1;
                old_lb = landing_boost;
                
                if((landing_boost >= 180)||(baro_alt >= old_alt)) { // detecting the ground effect      
		    land_complete = true;
                    return true;
                }
       ...
       //Serial.printf("icount %0.0f delta2 %d landing_boost %d\n",icount, delta2, landing_boost );
       return false;
}

static bool verify_landed()    // JLN update - check if truly landed, if yes disarm the motors
{  
//  Serial.printf("---> on the ground %d\n", ground);    
  if(baro_alt == old_galt) {
    ground ++;
    if(ground > 15) {
      init_disarm_motors(); 
      touchdown =  false;
...
/*
static bool verify_land()      // original R10 from Jason
{
...
static bool verify_loiter_turns()
{
	...
	// -----------------------------------------------
	if(abs(loiter_sum) > loiter_total / 1.5 ) {      // JLN update - reduced by 1.5 so as to get the true nbr of turns
		loiter_total 	= 0;
		loiter_sum		= 0;
