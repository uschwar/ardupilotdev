...
static void calc_loiter(int x_error, int y_error)    // AC v2.2 b2 and b2xp
{
...
/*
static void calc_loiter(int x_error, int y_error)    // AC v2.1.1 r9 version
{
	// East/West
...
static void update_crosstrack(void)
{
	...
	if (abs(wrap_180(target_bearing - original_target_bearing)) < 4500) {	 // If we are too far off or too close we don't do track following
		...
		crosstrack_error = sin(temp) * (wp_distance * g.crosstrack_gain * 100);	 // Meters we are off track line , JLN update
