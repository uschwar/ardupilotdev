...
#define THISFIRMWARE "ArduCopter V2.2b2xp3 JLN"
/*
ArduCopter Version 2.2 Beta
Authors:	Jason Short
...
// ================================================================================================
// Jean-Louis Naudin (JLN) updates: last update 2012-01-20
// modes tested in flight OK: STABILIZE, LOITER, ALT_HOLD, RTL, LAND
// modes tested in HIL simulation: STABILIZE, LOITER, ALT_HOLD, RTL, TAKEOFF, LAND, AUTO, GUIDED, CIRCLE
// very ALPHA version of the HELI-H1 mod: must be used only on simulator in HIL mode - for explorers only
//
// 2012-01-20: update for Helicopters type H-1, added throttle to collective curves (collective -> CH5)
// 2012-01-18: update for Helicopters type H-1 (i.e. Raptor, Vario Benzin) and HIL tested with AeroSIM-RC (Heli_H1.pde)
// 2012-01-18: disarming of the motors few seconds after the landing to be sure to be on the ground zero...
// 2012-01-17: AUTO-LAND TESTED IN FLIGHT on QRO quadcopter - smooth landing = OK
// 2012-01-17: added the ground effect detection on the final touchdown (commands_logic.pde) 
// 2012-01-16: nav_throttle is set to 0 for the final touchdown to avoid bouncing, see update_throttle_mode() in (arducopter.pde)
// 2012-01-16: loiter_total reduced by 1.5 for verify_loiter_turns() so as to get the desired nbr of turns (commands_logic.pde) 
// 2012-01-15: Tested autolanding (JLN update) on X-Plane and AeroSimRC = OK
// 2012-01-14: update on the g.crosstrack_gain * 100 (100 more) at navigation.pde
// 2012-01-14: Fine tuning of verify_land() function in (commands_logic.pde) for a smooth touchdown...
// 2012-01-13: I prefer using the update_GPS in the 10 Hz medium loop and NOT in the 50 Hz loop...
// 2012-01-12: added SW7 switch to CH7_LAND (control_modes.pde)
// 2012-01-12: GPS RED LED ON only if valid GPS + home_set (leds.pde)
// 2012-01-12: the baro_alt is used as altitude measurement for the LAND mode, takeof_point.alt contains the zero alt ref 
// 2012-01-12: store the home position + the takeoff_point where the motors are started (for LAND mode) (motors.pde)
// 2012-01-12: Reset all integrator at the arm_motors()
//
// more infos: http://diydrones.com/profile/JeanLouisNaudin
// ================================================================================================
...
static float icount = 1.0, lb;
static int16_t ground = 0;
...
static struct 	Location takeoff_point;    // JLN update
...
void loop()
{
	...
	//	if(GPS_enabled){        // JLN update  - Removed 
	//		update_GPS();
	//	}
			...
			if(manual_boost != 0){
				...
				if(invalid_throttle && motor_auto_armed == true){
					...
					//*/
                                ...
                                if(touchdown) {        // JLN update  - avoid bouncing during the touchdown
                                  nav_throttle = 0;
                                }
	...
	switch(control_mode){
		...
		case RTL:
			...
			if((wp_distance <= g.waypoint_radius) || check_missed_wp()){
                                old_alt = 0;
                                landing_boost 	= 0;
				// if this value > 0, we are set to trigger auto_land after 20 seconds
				set_mode(LOITER);
		...
		case POSITION:
			...
			if(auto_land_timer != 0 && (millis() - auto_land_timer) > 20000){  // JLN update reduced from 20000
