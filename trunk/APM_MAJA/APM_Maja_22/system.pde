...
void init_ardupilot()
{
	...
	Serial.printf_P(PSTR("\n\nInit ArduPilotMega 2.2d (JLN dev version for the MAJA UAV)"
						 "\n\nFree RAM: %lu\n"),
						 freeRAM());
	...
	if (g.compass_enabled==true) {
		...
		//compass.set_declination(ToRad(PARAM_DECLINATION));			// TODO fix this to have a UI	// set local difference between magnetic north and true north
	}
#else
	airspeed_enabled = true;
#endif
	...
	mavlink_system.compid = 1;	//MAV_COMP_ID_IMU;   // Waiting on an answer from Lorenz to figure out why this is only working for wp upload with value = 1
...
void print_flight_plan()
{
	...
	Serial.printf_P(PSTR("Loiter radius: %d\n\n"), (int)g.loiter_radius);
        Serial.printf_P(PSTR("---------------------------------\n"));
        Serial.printf_P(PSTR("Flight Plan:\n"));
...
void wp_list(struct Location *cmd, byte index)
{
	Serial.printf_P(PSTR("Wp #%d id:%d opt:%d p1:%d alt:%ld lat:%ld lon:%ld \n"),
		(int)index,
		(int)cmd->id,
		(int)cmd->options,
		(int)cmd->p1,
