...
void read_control_switch()
{
    ...
    if(switchPosition==0)   // in case of no receiver connected on the APM
    {   set_mode(AUTO);
    }
#endif
...
#else
                if(switchPosition==1) set_mode(RTL);
                if(switchPosition==3) set_mode(AUTO);
                if(switchPosition==5) set_mode(MANUAL);
#endif
