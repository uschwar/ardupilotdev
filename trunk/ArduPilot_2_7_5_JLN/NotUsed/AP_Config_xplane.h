...
#define THROTTLE_MAX 60   	    // (0-100) example: 70 = 56% throttle (lower this if your plane is overpowered)
...
#define REMEMBER_LAST_WAYPOINT 0 	//	If set 1 = will remember the last waypoint even if you restart the autopilot. 										// 	0 = Will start from WP 1 (not 0) every time you switch into AUTO mode. 
...
#define USE_AUTO_LAND 1		// If set to 1 then in AUTO mode roll will be held to zero and pitch to TAKE_OFF_PITCH until TAKE_OFF_ALT is reached
