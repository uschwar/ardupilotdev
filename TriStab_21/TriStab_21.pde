...
// Tricopter setup (total weight 446 g) :
// Frame: 3 aluminum tubes 8 mm diam and set at 120° and 380 mm lenght + 2 epoxy triangular plates
...
// - 1 transmitter Graupner MX16
// -------------------------------------------------------------------------
// Project start date : 10-28-10
// Version : 2.1 with GPS position Hold feature
// Last update: Nov 10, 2010 - JLN
//
...
/* Software version */
#define VER 2.10    // Current software version (only numeric values)
...
#define vers "TriStab, a VTOL-UAV stabilizer by Jean-Louis Naudin - v "
...
#if KNOB_TUNING == 0   
  K_aux=ROLL_PITCH_GAIN; // tuning of the ROLL_PITCH_GAIN gain with knob on ch6   1:Yes, 0: No
#endif 
...
// ROLL CONTROL   ============================================  
  ...
  // We also add a part that takes into account the command from user (stick) to make the system more responsive to user inputs
   ...
   roll_D = Stick_cmd_roll_diff*K_aux - ToDeg(Omega[0]);  // Take into account Angular velocity of the stick (command)
...
// PITCH CONTROL ==============================================
  ...
  // D term
  pitch_D = Stick_cmd_pitch_diff*K_aux - ToDeg(Omega[1]);
...
// YAW CONTROL WITH HEADING LOCK (if magnetometer installed) =================================
 ...
 // err_yaw = constrain(err_yaw,-60,60);  // to limit max yaw command... 
  ...
  Serial.print(") - yaw("); Serial.print(ToDeg(APM_Compass.Heading));
 
  Serial.print(") => err_Yaw="); Serial.print(err_yaw); 
  
  Serial.print(",  yaw_I=");  Serial.print(yaw_I); 
//  Serial.print(", Omega[2]=");   Serial.print(ToDeg(Omega[2])); 
  Serial.print(",  yaw_D=");   Serial.print(yaw_D);
  Serial.print(",  contral_yaw=");   Serial.print(control_yaw);
  Serial.print(",  hlock=");   Serial.println(hlock);
#endif
...
void setup(){
  ...
  pinMode(12,OUTPUT);     // Servo4  Rear Motor 
  pinMode(13,OUTPUT);     // Servo5  Yaw Servo  
 ...
 Servos[2].pin = 12;       // Rear motor
 Servos[3].pin = 13;       // Rudder servo
 ...
 // Magnetometer initialization
    ...
    APM_Compass.SetDeclination(ToRad(local_declination));
...
//    Serial.print("G_Dt:");            Serial.print(G_Dt);
      ...
      Serial.print("\tH:");             Serial.println(ToDeg(APM_Compass.Heading));   
    }
#endif    
    ...
    if (radio_status == 1){
      ...
      Stick_cmd_pitch_old = Stick_cmd_pitch;
      Stick_cmd_pitch = (ch2-Neutral[2])/13.0;
      Stick_cmd_pitch_diff = Stick_cmd_pitch-Stick_cmd_pitch_old;
      
      aux_float = (ch4-Neutral[4])/200.0;
     ...
     // when Yaw stick is release at the neutral position -> the current heading is stored as the TargetHeading
      ...
      AP_mode = 2;
      digitalWrite(7,HIGH);  // Yellow LED off
      digitalWrite(5,HIGH);  // Red Led
      ch3--;   // Descend  (Reduce throttle)
      if (ch3<MIN_THROTTLE)
        ch3 = MIN_THROTTLE;
      Stick_cmd_roll = 0;     // Stabilize to roll=0, pitch=0, yaw=0
      Stick_cmd_pitch = 0;
      Stick_cmd_yaw = 0;     
    ...
    if (ch3 > (MIN_THROTTLE+40))  // Minimun throttle to start control
      ...
      Servo_Timer2_set(1,MIN_THROTTLE);
      Servo_Timer2_set(2,MIN_THROTTLE);
      Servo_Timer2_set(3,Neutral[4]+control_yaw); 
    ...
    Serial.print("\t\tChL:");         Serial.print(chLEFT);
    Serial.print("\tChR:");           Serial.print(chRIGHT);
    Serial.print("\tChB:");           Serial.print(chBACK);
    Serial.print("\tchY:");           Serial.println(chYAW);            
...
{
         // TRICOPTER type heli CCPM 120° mixer ( 2 ailerons, 1 elevator)
         
      chLEFT  = ch3 + int( ((float)control_pitch/2) - ((float)control_roll * 0.87) );  
      chRIGHT = ch3 + int( ((float)control_pitch/2) + ((float)control_roll * 0.87) );   
      chBACK  = int(ch3 - control_pitch);  
      chYAW = ch4 + control_yaw;
      chYAW = constrain(chYAW, MIN_YAW, MAX_YAW);
      
      Servo_Timer2_set(0,chRIGHT);        // Right motor
      Servo_Timer2_set(1,chLEFT);         // Left motor
      Servo_Timer2_set(2,chBACK);         // Rear motor
      Servo_Timer2_set(3,chYAW);          // Rudder servo  (ch4)
}
