...
/*
ArduPilotMega 2.2f (JLN version) fully based on the ArduPilotMega v2.1
...
Updated version by Jean-Louis Naudin (April/May 2011) for the MAJA UAV and a full autonomous navigation in closed loop
+ GPS Rembizi emulator tested on the version v1.1.45
// Latest test flight 05-25-11

More infos about this current version at: http://diydrones.com/profile/JeanLouisNaudin
...
// Updated on 05-24-11: JLN Successful test on a 3.5 Km closed loop nav under flight plan - AirSpeed and throttle tuned
// Updated on 05-24-11: JLN Tested in flight v2.2f - Added the GBOOST for windy conditions (frontwind)
// Updated on 05-23-11: JLN Tested in flight v2.2e - Nav OK, WP alt lock correction
// Updated on 05-22-11: JLN control_mode updated for the GPS emulator, mode change with the transmitter switch in simulation
// Updated on 05-22-11: JLN use the latest library for the waypoint list set by the APM planner v 0.4.35
// Updated on 05-19-11: JLN New test in flight, minor bug corrected for the RTL
// Updated on 05-18-11: JLN New test in flight, minor bug corrected if closed loop nav is enabled
...
//#include "APM_Config_EMU.h"    // use the config file for the Rembizi GPS Emulator v1.1.45
#include "APM_Config.h"          // use the config file for the MAJA UAV (flight config)
...
#elif GPS_PROTOCOL == GPS_PROTOCOL_EMU
...
boolean airspeed_enabled = false;
...
int ready2nav = 0;                      // if GPS and Home is set ready2nav = 1;
boolean GPSnewdata = 0;
...
void fast_loop()
{
	...
	if (control_mode > MANUAL)
		stabilize();
                        ...
                        /* send Roll, Pitch, Throttle commands message to Emulator */                         
                         GPS_enabled = true;
                         output_gpsemulator();
        #endif
...
Serial.println(tempaccel.z, DEC);
		...
		case 1:
			...
			if((g_gps->new_data == true) || (GPSnewdata == true)){
		...
		case 3:
			...
			// XXX this should be a "GCS medium loop" interface
				...
				gcs.send_message(MSG_ATTITUDE);     // Sends attitude data
                         ...
                         #if ADS_PROTOCOL == ADS_PROTOCOL_JLN  ||  GPS_PROTOCOL == GPS_PROTOCOL_EMU     
                        /* send attitude message to JLN ardustation */
                         print_attitude2ADS();
	 		#endif
		...
		case 2:
			...
			// XXX this should be a "GCS slow loop" interface
				...
				gcs.send_message(MSG_CPU_LOAD, load*100);
                         ...
                         #if ADS_PROTOCOL == ADS_PROTOCOL_JLN  || GPS_PROTOCOL == GPS_PROTOCOL_EMU      
                        /* send position message to JLN ardustation */
                          print_position2ADS();                         
                        #endif                        
