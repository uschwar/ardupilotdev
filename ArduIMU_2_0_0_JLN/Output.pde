...
{    
	...
	Serial.print(",");
		...
		Serial.print (",MGH:");
		Serial.print((int)ToDeg(compass.heading));
		Serial.print (",");
	#endif
...
//		Serial.print(press>>2);       // Convert into Pa       
		...
		if(GPS.new_data==1) {
			...
			Serial.print(",COG:");
	#if USE_MAGNETOMETER == 1
                        ground_course = ToDeg(compass.heading);
	                if (ground_course < 0)	ground_course += 360;
		        Serial.print((int)ground_course);
        #else
			...
			Serial.print(GPS.ground_speed/100);
		        Serial.print(",STA:");
		        Serial.print(GPS.status(), DEC);
		        Serial.print(",VAR:");
		        Serial.print(GPS.valid_read, DEC);
			Serial.print(",FIX:");
	...
	if(GPS.new_data==1) {
		...
		IMU_buffer[13]=(tempint>>8)&0xff;
                ...
                #if USE_MAGNETOMETER == 1
                    ground_course = ToDeg(compass.heading) * 100;
	            if (ground_course < 0)	ground_course += 36000;
		 #else
                        ground_course = GPS.ground_course;
                #endif 
        
		tempint=ground_course;   // course in degreees * 100 in 2 bytes
