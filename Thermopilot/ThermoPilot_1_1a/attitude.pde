...
void stabilize()
{
	...
	// ---------------------------------------------
	servo_out[CH_ROLL]  = PIDcompute(nav_roll, deltaMiliSeconds, CASE_SERVO_ROLL);
	servo_out[CH_PITCH] = PIDcompute(nav_pitch, deltaMiliSeconds, CASE_SERVO_PITCH);  
...
void calc_nav_roll()
{
	...
	nav_gain_scaler = ground_speed / (float)(AIRSPEED_CRUISE * 100);
